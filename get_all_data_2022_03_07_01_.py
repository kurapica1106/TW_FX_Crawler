...
def option_daily_summary(y1, m1, d1, y2, m2, d2):
    ...
    sql = " \
        select YMD, Year, Month, Day, 交易時段, CP, sum(V), sum(C * V), sum(OI), sum(結算價 * OI) from Option_TXO_Daily_Detail \
        where YMD between %d and %d\
        group by YMD, 交易時段, CP \
        " % (ymd1, ymd2)
    cur.execute(sql)
    results = cur.fetchall()
    dfs_totalc = [["%d_%s_%s_0" % (r[0], r[4], r[5])] + [r[0], r[1], r[2], r[3], r[4], r[5]] + [int(), int(), int(), int()] + [r[6], r[7], r[8], r[9]] for r in results]
    
    dfs = dfs + dfs_totalc
    dfs.sort()
    
    table_name = "Option_TXO_Daily_Summary"
    sql = sql_insert_n_values(table_name, 15)
    cur.executemany(sql, dfs)
    
    conn.commit()
    conn.close()

option_daily_summary(2019, 1, 1, 2022, 3, 4)
#to_database_many("future", "TX", 2019, 1, 1, 2022, 3, 4)
#to_database_many("future", "MTX", 2019, 1, 1, 2022, 3, 4)
#to_database_many("option", "TXO", 2019, 1, 1, 2022, 3, 4)